...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_quiz') {
        ...
        if ($titre) {
            db()->prepare("INSERT INTO questionnaires (created_by, titre, description) VALUES (?,?,?)")
               ->execute([$user['id'], $titre, $desc]);
            $qid = db()->lastInsertId();
    ...
    if ($action === 'submit_answers') {
        ...
        foreach ($reps as $question_id => $answer_index) {
            // Supprimer ancienne réponse si existe
...
input[type=text],textarea{width:100%;background:transparent;border:1px solid var(--border);color:var(--text);font-family:'DM Mono',monospace;font-size:.8rem;padding:.7rem .9rem;outline:none;transition:border .2s}
...
<div class="qview-header">
  ...
  <div class="qview-meta"><?= h($quiz['display_name']) ?> · <?= count($questions) ?> <?= t('questions','вопросов') ?></div>
...
<?php if ($already): ?>
  <!-- Afficher les réponses comparées -->
  ...
  <?php
    ...
    $users_list = db()->query("SELECT * FROM users")->fetchAll();
  ?>
  <div class="qview-block">
    ...
    <div class="answers-compare">
      ...
      <div class="ans-box <?= $all_same&&$rep?'same':'' ?>">
        ...
        <div class="what"><?= $ai!==null ? $letters[$ai].'. '.h($opt) : '<span style="color:var(--muted)">—</span>' ?></div>
      </div>
      <?php endforeach; ?>
    </div>
