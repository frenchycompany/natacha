...
// Ensure column
try { db()->query("SELECT app_pin FROM users LIMIT 1"); } catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($setup) {
        ...
        if (preg_match('/^\d{4}$/', $pin)) {
            ...
            db()->prepare("UPDATE users SET app_pin=? WHERE id=?")->execute([$hash, $user['id']]);
            ...
            $error = $lang === 'ru' ? 'Введите 4 цифры' : 'Entrez 4 chiffres';
        }
    } else {
        // Verifying PIN
        $stmt = db()->prepare("SELECT app_pin FROM users WHERE id=?");
...
<div class="lock-card">
    ...
    <div class="lock-sub">
        <?php if ($setup): ?>
            <?= $lang==='ru' ? 'Создайте код из 4 цифр для защиты приложения' : 'Créez un code à 4 chiffres pour protéger l\'appli' ?>
        <?php else: ?>
